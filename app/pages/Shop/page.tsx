...
const ProductCard = ({product, isList, onFavorite, isFavorite}: any) => {
    ...
    const handleFavoriteAndCart = () => {
        const newFav = {...favorite}; // Copy the current state to avoid direct mutation
        newFav[product._id] = !newFav[product._id]; // Toggle favorite status using product._id as key
        setFavorite(newFav);

        const isFavourite = newFav[product._id];
        handleAddToCart(product._id, 0, isFavourite); // Add to cart with favorite status
    };
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                console.log("prouct data yaaaaaaaaaaaaaaaaaaaaaaa raouf", productsData)
            } catch (error) {
