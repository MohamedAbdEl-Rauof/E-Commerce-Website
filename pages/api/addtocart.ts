  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!cart) {
        ...
        return res
          .status(200)
          .json({ message: "Category added to the cart successfully" });
      } else {
      ...
      res.status(200).json({ message: "Category added to cart successfully" });
    } catch (error) {
    ...
    try {
      ...
      const existingProductIndex = cart.info.findIndex((item: CartItem) =>
      ...
      res
        .status(500)
        .json({ message: "An error occurred while updating cart" });
    }
